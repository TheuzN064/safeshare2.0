...
// Estatísticas gerais
$totalSenhas = (int) $pdo->query('SELECT COUNT(*) FROM logins WHERE id_usuario = 1')->fetchColumn();
$totalCartoes = (int) $pdo->query('SELECT COUNT(*) FROM cartoes WHERE id_usuario = 1')->fetchColumn();
$totalBancos = (int) $pdo->query('SELECT COUNT(*) FROM bancos')->fetchColumn();

$categoriasResumo = $pdo->query('SELECT c.nome, c.cor_hex, COUNT(l.id) AS total FROM categorias c LEFT JOIN logins l ON l.id_categoria = c.id GROUP BY c.id, c.nome, c.cor_hex')->fetchAll();
$bancosResumo = $pdo->query('SELECT b.nome, COUNT(c.id) AS total FROM bancos b LEFT JOIN cartoes c ON c.id_banco = b.id GROUP BY b.id, b.nome')->fetchAll();
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
